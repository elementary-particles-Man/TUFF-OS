...
BUILDROOT_DIR="$HOME/tuff-buildroot"
...
if [ ! -d "$BUILDROOT_DIR" ]; then
    ...
    mkdir -p "$HOME"
    wget -qO- https://buildroot.org/downloads/buildroot-$BUILDROOT_VER.tar.gz | tar xz -C "$HOME"
    mv "$HOME/buildroot-$BUILDROOT_VER" "$BUILDROOT_DIR"
fi
